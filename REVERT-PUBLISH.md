...
| 2026-09-22 publish | `6c4f70f` | Blue scoped free-items notification added to the Cart tab (app.js v136) (revert target) |
| 2026-09-23 publish | `f29dc26` | Free-items modal: Promotions column + per-reward promotions + Offers lead column removed (app.js v141) |

## To revert the live site back to what it was before this publish

```bash
cd "/Users/lakshmi.tv/Desktop/Free Items/free-items"
git push origin 6c4f70f:gh-pages --force
```

That restores commit `6c4f70f` as the live site (allow 1–2 min for GitHub Pages
to rebuild). Nothing else — your `main` branch and local source are untouched.

To re-publish the newer (Promotions-column) version again afterward:
